    ...
    async upload() {
      ...
      if(response.status === OK) {
        ...
        if(response.data.errors.name) {
          this.message.name = response.data.errors.name[0];
        }
        if(response.data.errors.price) {
          this.message.price = response.data.errors.price[0];
        }
        if(response.data.errors.file) {
          this.message.image = response.data.errors.file[0];
        }
      }else {
